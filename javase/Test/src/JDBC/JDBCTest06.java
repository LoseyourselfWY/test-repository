...
要向用户信息不参与SQL语句的编译，那必须使用java.sql.PreparedStatement
...
2、对比一下Statement和PreparedStatement
  ...
  - Statement是编译一次执行一次，PreparedStatement是编译一次可执行N次，效率较高
  - PreparedStatement会在编译阶段进行类型的安全检查

  综上PreparedStatement使用较多，只有在业务方面必须要求SQL注入时才使用Statement
 * */
...
//        3、获取预编译的数据库操作对象
//            SQL语句的框子，其中一个？代表一个占位符，一个？将来接受一个"值"，注意：占位符不能够使用单引号括起来
